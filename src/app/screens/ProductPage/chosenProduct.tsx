...
export function ChosenProductPage(props: any) {
    ...
    const dishRelatedProcess = async () => {
        try {
            ...
            const restaurant = await companyApiService.getChosenCompany(
                product.company_mb_id
            );
            setChosenCompany(restaurant);  // redux iga setChosenRestaurant ni path qilyabman
        } catch (err) {
            ...
            <Container className="dish_container" sx={{display: "flex"}}>
                ...
                <Stack className={"chosen_dish_info_container"}>
                    <Box className={"chosen_dish_info_box"}>
                        ...
                        <span className={"resto_name"}>{chosenCompany?.mb_nick}</span>
